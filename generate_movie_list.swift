...
func fetchTopMoviesForEvents() async {
    ...
    let selectedMetas = Array(validMetas)
    ...
    let configData = EventsConfigData(
        movies: selectedMetas,
        cycle_duration_hours: 200, // Based on cycle calculation
        buffer_between_movies_seconds: 600,
        epoch_timestamp: 1704067200,
        generated_at: ISO8601DateFormatter().string(from: Date())
    )
    
    if let jsonData = try? encoder.encode(configData),
       let jsonString = String(data: jsonData, encoding: .utf8) {
        ...
        let filename = "update_movie_events_v\(cycleNumber).sql"
        ...
        print("✅ SQL migration saved to: \(filename)")
        print("🚀 Run this SQL in your Supabase SQL Editor to update the movie list!")
    }
}
