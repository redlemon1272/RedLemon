...
for INSTALLER in ""; do
    if [[ -f "" ]]; then
        ...
        if ! grep -q "lsregister" ""; then
             report "ERROR" "Landmine #156" "Missing 'lsregister' refresh in installer. This causes Intel 'prohibited' sign regressions." "" "0" "Missing lsregister call"
        fi
        # Intel: FinderInfo
        if ! grep -q "FinderInfo" ""; then
             report "WARNING" "Landmine #156" "Missing 'FinderInfo' xattr cleanup. Stale Finder bits can block bundle execution on Intel." "" "0" "Missing xattr -d com.apple.FinderInfo"
        fi
        # Silicon: Quarantine
        if ! grep -q "com.apple.quarantine" ""; then
             report "ERROR" "Landmine #156" "Missing quarantine removal in installer. Apple Silicon will block execution." "" "0" "Missing xattr -rd com.apple.quarantine"
        fi
    fi
done
...
# =============================================================================
# CHECK 41: Race Condition in Async Task Cleanup (Landmine #159)
# =============================================================================
# Trigger: MPVPlayerViewModel doesn't capture values locally before async Task.
print_header "Check 41: Race Condition in Async Task Cleanup (Landmine #159)"
...
if [[ -f "$MPV_PLAYER_VM" ]]; then
    ...
    if ! grep -q "currentMediaItem = appState?.player.selectedMediaItem" "$MPV_PLAYER_VM"; then
        report "ERROR" "Landmine #159" "Race Condition Risk: MPVPlayerViewModel MUST capture 'selectedMediaItem' locally in loadStream() before async Task executes. Use 'self.currentMediaItem = appState?.player.selectedMediaItem'." "$MPV_PLAYER_VM" "0" "Missing local capture of selectedMediaItem"
    else
    ...
    if ! grep -q "currentQuality = qualityEnum.rawValue" "$MPV_PLAYER_VM" && ! grep -q "currentQuality = qualityStr" "$MPV_PLAYER_VM"; then
        report "WARNING" "Landmine #159" "Race Condition Risk: MPVPlayerViewModel should capture quality locally. Use 'self.currentQuality = qualityEnum.rawValue'." "$MPV_PLAYER_VM" "0" "Missing local capture of quality"
    fi
fi

# =============================================================================
# CHECK 42: Dictionary Ordering in SwiftUI (Landmine #160)
# =============================================================================
# Trigger: Using Array(dict.values) without explicit sorting.
print_header "Check 42: Dictionary Ordering in SwiftUI (Landmine #160)"
...
if [[ -f "$BROWSE_VM" ]]; then
    ...
    if grep -q "latestBySeries.values" "$BROWSE_VM"; then
        if ! grep -q "latestBySeries.values.sorted" "$BROWSE_VM"; then
            report "ERROR" "Landmine #160" "Dictionary Ordering Risk: filteredHistoryItems uses dictionary values without explicit sorting. MUST add '.sorted { \$0.lastWatched > \$1.lastWatched }' to prevent UI items from trading places." "$BROWSE_VM" "0" "Missing explicit sort on dictionary values"
        else
