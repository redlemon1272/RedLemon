...
class LobbyEventRouter: ObservableObject {
    ...
    private func handleLobbyJoin(_ syncMessage: SyncMessage) async {
        ...
        if viewModel.isHost {
             ...
             // (Internal Note Security Check #13: ephemeral state must be re-synced on join)
             await broadcastCurrentVotes()

             // CRITICAL FIX (Security Check #90): Sync participants to AppState immediately
             ...
             // Filter out self-echo to prevent duplicates since sender adds locally (Security Check #61)
