...
class MPVWrapperFixed: ObservableObject {
    ...
    private func startMemoryMonitoring() {
        memoryMonitorTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: true) { [weak self] _ in
            ...
            if kerr == KERN_SUCCESS {
                ...
                if usedMB > 200.0 {
                    print("⚠️ High memory usage detected - clearing MPV buffers")
                    strongSelf.clearMPVBuffers()
                }
            }
        }
    }

    private func clearMPVBuffers() {
        guard let handle = mpvHandle, isInitialized else { return }

        print("🧹 Clearing MPV video buffers...")

        // Cycle video codec to force buffer clear
        mpv_command_string(handle, "cycle video")
        mpv_command_string(handle, "cycle video") // Return to normal

        // Clear cache if possible
        mpv_command_string(handle, "set cache-secs 1")
    ...
    // MARK: - Enhanced Event Polling with Better Cancellation

    private func pollEvents() async {
        guard let handle = mpvHandle else { return }

        while !Task.isCancelled {
            let event = mpv_wait_event(handle, 0.1) // Shorter timeout for faster cancellation
    ...
    private func handleMPVEvent(eventId: mpv_event_id, eventPtr: UnsafePointer<mpv_event>) {
        ...
        case MPV_EVENT_START_FILE: isBuffering = true
        case MPV_EVENT_FILE_LOADED: updateDuration()
        case MPV_EVENT_PLAYBACK_RESTART: isBuffering = false; isPlaying = true
        case MPV_EVENT_END_FILE: isPlaying = false
        case MPV_EVENT_IDLE: isBuffering = false
        default:
