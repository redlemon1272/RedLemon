-- Revert Security Fixes (Incompatible with Anon Key Auth)

-- 1. Restore assign_payment_address with p_user_id
...
-- 2. Restore room_heartbeat with p_user_id
...
-- Drop the broken versions (which had fewer args)
DROP FUNCTION IF EXISTS public.room_heartbeat(text);
DROP FUNCTION IF EXISTS public.assign_payment_address(text);
