...
# Get the directory where this script is located (handling symlinks)
...
analyze_build_failure() {
    ...
    if grep -q "swift-collections" "$PROJECT_ROOT/.build/debug.yaml" 2>/dev/null; then
        log_warning "Detected swift-collections related issues. This may require dependency version updates."
    fi

    if grep -q "OrderedCollections" "$PROJECT_ROOT/.build/debug.yaml" 2>/dev/null; then
    ...
    echo "   1. Run: rm -rf .build && scripts/start-production.sh"
...
launch_app() {
    ...
    xattr -d com.apple.quarantine "$PROJECT_ROOT/build/RedLemon.app" 2>/dev/null || true

    # Verify app binary exists
    if [ ! -f "$PROJECT_ROOT/build/RedLemon.app/Contents/MacOS/RedLemon" ]; then
        log_error "RedLemon binary not found at $PROJECT_ROOT/build/RedLemon.app/Contents/MacOS/RedLemon"
        return 1
    fi
